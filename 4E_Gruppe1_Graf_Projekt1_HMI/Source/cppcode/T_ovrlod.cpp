...
_BOOL overload_PICTUREMASK(_PICTUREMASK*, _VARLABEL*);
...
void overload_IO(_IO *p, _VARLABEL *ov)
{
  ...
  if(p->lock_overload == _FALSE)
  {
    ...
    overload_VARLABEL(&p->variable, ov);
...
void overload_MEM_INPUT(_MEM_INPUT *p, _VARLABEL *ov)
{
  _UWORD i;
  _INPUT *pi;

  for(i=0, pi=p->ptr; i<p->no; i++, pi++)
    overload_IO(&pi->io, ov);
}

void overload_MEM_OUTPUT(_MEM_OUTPUT *p, _VARLABEL *ov)
{
  _UWORD i;
  _IO    *pio;
...
*/
void overload_MEM_OBJECT(_MEM_OBJECT *p, _VARLABEL *ov)
{
  _UWORD  i;
  _OBJECT *po;

  for(i=0, po=p->ptr; i<p->no; i++, po++)
    overload_OBJECT(po, ov);
}
...
_BOOL overload_PICTUREMASK(_PICTUREMASK *p, _VARLABEL *ov)
{
  if(valid_VARLABEL(ov) == _TRUE)
  {
    ...
    overload_MEM_OBJECT(&p->memobject, ov);
    overload_MEM_BITMAP(&p->membitmap, ov);
    overload_MEM_INPUT(&p->meminput, ov);
    overload_MEM_OUTPUT(&p->memoutput, ov);
