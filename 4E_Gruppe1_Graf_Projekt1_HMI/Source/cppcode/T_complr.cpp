...
_IVALUE _Compiler::GetVarlabel(void)
{
  ...
  if(Error == ERROR_NONE)
  {
    ...
    if(VarList.GetValue(&vl, &retcode, _TRUE) == _FALSE)
      retcode = 0;
  }
...
_IVALUE _Compiler::GetChkBit(void)
{
  ...
  {
    sno     = *((_UWORD*)Source);
    retcode = ((Sequencer.Call(sno, _FALSE) == _TRUE)? 1 : 0);
    Source += 2;
  }
...
_IVALUE _Compiler::Run(_COMPCODE *p, _BOOL *err)
{
  _IVALUE retcode = 1; 
...
_BOOL _Sequencer::Call(_UWORD sno, _BOOL invert)
{
  if(sno < No)
  {
   #ifdef _STATICCHKBIT
    ...
    {
      retcode = CallCompiler(&Ptr[sno], invert);
    ...
    return retcode;
   #else
    return CallCompiler(&Ptr[sno], invert);
   #endif
  }
...
_BOOL _Sequencer::CallCompiler(_COMPCODE *p, _BOOL invert)
{
  ...
  if(p->ptr != NULL)
  {
    retcode = (Compiler.Run(p, &error) != 0)? _TRUE : _FALSE;
    if(error == _TRUE)
      retcode = _TRUE;
