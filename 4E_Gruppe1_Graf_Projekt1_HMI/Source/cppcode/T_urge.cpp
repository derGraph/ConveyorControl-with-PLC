...
_DOIT getfocus_THEURGE(_EDITOR *pedi)
{
  _DOIT retcode = _IDLE;
  ...
  if(pedi->input.io.theurge != NULL)
  {
    switch(pedi->input.io.theurge->urgetype)
    {
      case _URGE_SLIDER : retcode = getfocus_SLIDER(pedi);
        break;
    }
  }
...
{
  _BOOL retcode = VarList.GetSystemData(res, var, &pio->variable, _FALSE, dolimit);
...
_BOOL _SubPicture::Out_THEURGE_Bar(_IO *pio, _RESULT *pres, _VARIABLE *pv)
{
  ...
  for(i=0,j=1; (i<p->memvarlabel.no)&&(draw==_TRUE); i++,j++)
  {
    ...
    if(VarList.GetSystemData(&result[j], &var, &p->memvarlabel.ptr[i], _FALSE, _TRUE) == _TRUE)
      draw = _TRUE;
  }
...
_BOOL _SubPicture::Out_THEURGE_ScreenClean(_IO *pio, _RESULT *pres, _VARIABLE *pv)
{
  ...
  {
    _RESULT result;
    if(VarList.GetSystemData(&result, &varia, &p->memvarlabel.ptr[0], _FALSE, _TRUE) == _TRUE)
    {
        if(result.value < 1)
          result.value = 1;
...
_BOOL _SubPicture::Out_THEURGE_Scale(_IO *pio, _RESULT *pres, _VARIABLE *pv)
{
  ...
  if(p->memvarlabel.no >= 4)
  {
    ...
    // get minval
    if(VarList.GetSystemData(&res[1], &var, &p->memvarlabel.ptr[0], _FALSE, _TRUE) == _FALSE)
      init_RESULT(&res[1]);
    ...
    // get maxval  
    if(VarList.GetSystemData(&res[2], &var, &p->memvarlabel.ptr[1], _FALSE, _TRUE) == _FALSE)
      init_RESULT(&res[2]);
    ...
    // get graduates  
    if(VarList.GetSystemData(&res[3], &var, &p->memvarlabel.ptr[2], _FALSE, _TRUE) == _FALSE)
      return(_FALSE);
    
    // get graduates  
    if(VarList.GetSystemData(&res[4], &var, &p->memvarlabel.ptr[3], _FALSE, _TRUE) == _FALSE)
      return(_FALSE);
