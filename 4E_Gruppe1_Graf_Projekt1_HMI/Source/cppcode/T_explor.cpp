  ...
  if(txt != NULL)
  {
    ...
    else
    {
      ...
      if(cut != 0xFFFF)
      {
        ...
        _KOORD xx = (_KOORD)pex->xoffset;
        if(sign == ID_DRIVE)
          xx = StrWidth(&txt[1], sizeof(_ASCII));

        txt[cut] = CUT_INFO;
        OutTextXY(x + xx, y, &txt[cut+1], att, col, sizeof(_ASCII));
      }
      else
      {
        OutTextXY(x, y, &txt[1], att, col, sizeof(_ASCII));
...
_WORD filexplo_sort(_SORTA *p1, _SORTA *p2)
{
...
_UWORD filexplo_read_part(_FILEXPLORER *pex, _AsciiPuffer *pls, const _ASCII *dpne, _UDWORD inat, _UDWORD exat)
{
    ...
    if(name[1] != 0)
    {
      ...
      else
      {
        ...
        if(pex->ShowDate == _TRUE)
        {
          ...
          format_date(&info[1], &dte, pex->FormatDate);
        }
  ...
  if(pls->GetNo() != oldno)
    sort_ASCII(pls->GetText(oldno), pls->GetNo()-oldno, (_QSORTFUNCT)&filexplo_sort);
