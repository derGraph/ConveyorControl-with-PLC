...
_BOOL check_varno(_UDWORD varno)
{
  if(varno != DEFSCOPE)
  {
    if((varno < VarList.GetNo()) || (varno >= VARNO_INSERTFLAG))
      return _TRUE;
  }
  return _FALSE;
}
...
{
  return add_button_intern(mp, pstyle, pr, stschemeno, fktno_press, fktno_release, varno, imageno_press, imageno_release, text_press, text_release, chrsize, act_momentary, accessno, hotkeycode, overload, setval, resetval, imagealign, _NOKEY, _NOKEY, _NOKEY, _TIPSINGLE, _SELECTOR_ONLY_HID);
}

extern "C" _BOOL add_key(_MY_PICTURE *mp, _STYLESHEET *pstyle, _ROOM *pr, _UDWORD imageno, void *text, _UWORD chrsize, _ATTRIB imagealign, _UWORD keycode, _UWORD keycode_shift, _UWORD keycode_altgr)
{
  return add_button_intern(mp, pstyle, pr, DEFSCOPE, DEFSCOPE, DEFSCOPE, DEFSCOPE, imageno, imageno, text, text, chrsize, _TRUE, 0, 0, DEFSCOPE, 1, 0, imagealign, keycode, keycode_shift, keycode_altgr, _TIPCOMBI, _SELECTOR_ONLY_HID);
}
