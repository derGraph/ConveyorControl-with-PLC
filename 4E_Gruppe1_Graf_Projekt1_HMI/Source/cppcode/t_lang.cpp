...
{
  // information steht im projektfile und müsste ab compilerversion >= 36 nicht
...
_BOOL _Language::ReadLanguageInfoSub(_ASCII *dpne, _LANGUAGE_INFO *p, _MEM_NO *pfd)
{
  ...
  if(File.ReadHeader() == _TRUE)
  {
    if(File.Read(&offset, sizeof(_UDWORD)) == _TRUE)
    {
      ...
      {
        File.Read(&chrsize, sizeof(_UBYTE)); // zeichengrösse, gilt nur für aliasname
...
_BOOL _Language::LoadLanguage(_UWORD language_no)
{
  ...
  {
    // bei ungültiger sprachnummer auf sprache 0 schalten
    language_no = 0;
