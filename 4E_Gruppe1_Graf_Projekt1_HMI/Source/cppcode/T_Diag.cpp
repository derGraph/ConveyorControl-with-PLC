...
void popup_varview_line(_Window *Pw, _PopUp *Pp, _UDWORD no, _ROOM *r, _COLOR col, _BOOL background)
{
  ...
  {
    ret = _TRUE;
    copy_LSE_VARINFO(&ret, &vi, (_LSEVARINFO*)pm->pointer.PtrData, _TRUE);
...
//       #ifdef UC_GLOBAL_VAR
      ...
      case 7:
        ...
        if(vi.vartype == BINARY)
        {
          if(VarList.GetZero(vi.POWPRUIC) == _TRUE)
...
//               #ifdef UC_GLOBAL_VAR
               #if defined(UC_GLOBAL_VAR) || defined(UC_GLOBAL_VAR_REFLIST)
                if(var.info.POWPRUIC & VARBIT_VARGLOBAL)
                {
                  state = get_global(&value, &var.info);
                }
                else
               #endif
                {
...
void popup_unit_line(_Window *Pw, _PopUp *Pp, _UDWORD no, _ROOM *r, _COLOR col, _BOOL background)
{
  ...
  {
    OutTextXY(r->xy1.x, (r->xy1.y + r->xy2.y) / 2, "No    Dig Dez Formula Fix Text", T_UNDERLINE|T_COPY|T_SOLID|T_PROP|T_LEFTBOUND|T_MIDBOUND, col, sizeof(_ASCII));
  }
  else
  {
    no -= 1;
...
void popup_sequence_run(_Window *Window, _PopUp *PopUp, _EVENT *pev)
{
  ...
  if((pev->ftype == _EVENT_KEYPRESS) && (pev->scancode == _ENTER))
  {
    ...
    {
      if(Sequencer.Get(&compcode, no) == _TRUE)
        out_runtext(PopUp, (void*)((Sequencer.CallCompiler(compcode, _FALSE) == _TRUE)? "{TRUE}":"{FALSE}"), sizeof(_ASCII));
