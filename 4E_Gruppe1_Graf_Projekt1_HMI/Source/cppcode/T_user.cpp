...
void make_offlinewindow_hid(_MY_PICTURE *mp, _EVENT *pe)
{
  _ROOM  room;
  _DOT   dot;
  _EVENT evnt;
...
void resetwindow_ready(_MY_PICTURE *mp)
{
  ...
  if(mp->JustProgram == _TRUE)
  {
    ...
    room = mp->Room;
   #ifdef HID
    OutMultiTextAlign((void*)"ATTENTION !!|Reset CPU", 0, &room, fnt, T_MIDBOUND|T_CENTERBOUND|T_COPY|T_SOLID|T_PROP, _EDGEFRAME, WHITE, MakeColor_Makro(LIGHTRED, LIGHTRED), MakeColor_Makro(WHITE, RED), sizeof(_ASCII));
    pos_2button(&room, &mp->Room, _TRUE);
    OutMultiTextAlign((void*)"<ENTER>...reset", 0, &room, fnt, T_MIDBOUND|T_CENTERBOUND|T_COPY|T_SOLID|T_PROP, _3DFRAME, WHITE, MakeColor_Makro(LIGHTRED, LIGHTRED), MakeColor_Makro(WHITE, RED), sizeof(_ASCII));
    pos_2button(&room, &mp->Room, _FALSE);
    OutMultiTextAlign((void*)"<ESC>...cancle", 0, &room, fnt, T_MIDBOUND|T_CENTERBOUND|T_COPY|T_SOLID|T_PROP, _3DFRAME, WHITE, MakeColor_Makro(LIGHTRED, LIGHTRED), MakeColor_Makro(WHITE, RED), sizeof(_ASCII));
   #else
    OutMultiTextAlign((void*)"ATTENTION !!|Reset CPU|Press <ENTER>", 0, &room, fnt, T_MIDBOUND|T_CENTERBOUND|T_COPY|T_SOLID|T_PROP, _EDGEFRAME, WHITE, MakeColor_Makro(LIGHTRED, LIGHTRED), MakeColor_Makro(WHITE, RED), sizeof(_ASCII));
   #endif
  }
...
//  {
//    if(P_Picture != NULL)
//      offlinewindow_start(P_Picture->GetMyPicture());
//  }
...
/*
_UDWORD RauteScan(char *ptr)
{
  _UDWORD retcode = 0;
  _UDWORD len = StrLen(ptr, 1);
  ...
  while(len)
  {
    ...
    {
      case 'U' :
        if(MemCmp(ptr, "UserFcts", 8) == 0)
          return 0;
        break;
      case 'S' :
        if(MemCmp(ptr, "StoreMethod", 11) == 0)
          return 0;
        break;
      case 'p' :
      ...
      case 'P' :
        ...
        break;
      case 'd' :
        if(MemCmp(ptr, "define", 6) == 0)
          return 0;
        break;
      case 'C' :
        if(MemCmp(ptr, "CmdTable", 8) == 0)
          return 0;
        break;
    }
...
void DoRaute(void)
{
  ...
  if(Dst.Open(SAVE, "RAM@E:\\this.TXT") == _TRUE)
  {
    if(Src.Open(LOAD, "E:\\USR1.TXT") == _TRUE)
    {
      while(Src.ReadCrLf(tmp, 2000, 1) == _TRUE)
      {
        if(RauteScan(tmp) == 3)
        {
          Dst.Write(tmp, StrLen(tmp, 1));
          tmp[0] = 13;
          tmp[1] = 10;
          Dst.Write(tmp, 2);
        }
...
//    case 14444 : DoRaute(); // todo: entfernen
//              break;
...
void found_something_by_hid(_FOUNDHID hp)
{
 #ifndef CUTOFF_USER
